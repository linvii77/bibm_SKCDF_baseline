...
def normalize_data_id(data_id, task="synapse"):
    data_id = data_id.strip()
    if not data_id or data_id.startswith('#'):
        return None
    if task == "synapse":
        if data_id.startswith('case'):
            data_id = data_id[4:]
        if '_slice' in data_id:
            data_id = data_id.split('_slice')[0]
    return data_id


def _resolve_split_path(splits_dir, split):
    split_aliases = {
        'eval': ['eval', 'val'],
        'labeled_20p': ['labeled_20p', 'labeled_cases'],
    }
    for name in split_aliases.get(split, [split]):
        path = os.path.join(splits_dir, f'{name}.txt')
        if os.path.exists(path):
            return path
    return os.path.join(splits_dir, f'{split}.txt')


def read_list(split, task="synapse"):
    config = Config(task)
    splits_dir = getattr(config, 'splits_dir', os.path.join(config.save_dir, 'splits'))
    split_path = _resolve_split_path(splits_dir, split)

    ids_list = []
    with open(split_path) as f:
        for line in f:
            data_id = normalize_data_id(line, task=task)
...
def read_data(data_id, task, nifti=False, test=False, normalize=False):
    config = Config(task)
    data_id = normalize_data_id(data_id, task=task)
    data_dir = getattr(config, 'data_dir', os.path.join(config.save_dir, 'npy'))
    im_path = os.path.join(data_dir, f'{data_id}_image.npy')

    lb_path = os.path.join(data_dir, f'{data_id}_label.npy')
...
def fetch_data(batch, labeled=True): #b,c,d,h,w   b,d,h,w
