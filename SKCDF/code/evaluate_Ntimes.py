...
if __name__ == '__main__':
    ...
    txt_path = os.path.join(config.log_dir, args.exp.lstrip('/'), "evaluation_res.txt")
    # print(txt_path)
    print("\n Evaluating...")
    fw = open(txt_path, 'w')
    for fold in range(1,args.folds + 1):
        ...
        for idx, data_id in enumerate(tqdm(ids_list)):
            ...
            pred = read_nifti(os.path.join(config.log_dir, args.exp.lstrip('/'), "fold"+str(fold), "predictions_"+str(args.cps),f'{data_id}.nii.gz'))
            ...
            label=np.load(os.path.join(config.data_dir, f'{data_id}_label.npy'))
